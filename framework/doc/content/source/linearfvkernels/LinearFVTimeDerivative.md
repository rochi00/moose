...
!equation
\int\limits_{V_C} c\,\frac{\partial u}{\partial t} dV \approx c_C \left(\frac{\partial u}{\partial t}\right)_C V_C~,

where $\left(\frac{\partial u}{\partial t}\right)_C$ and $V_C$ are the time derivative of the
field value at the cell center and the cell volume, respectively.

By default the multiplier sits inside the derivative, so the kernel assembles
$\partial (cu)/\partial t$. The difference between that and $c\,\partial u/\partial t$ is which
state each multiplier is taken at. A time integrator with weights $w_k$ forms the derivative from a
run of solution states, and the conservative form pairs each state with the multiplier at the same
time,
...
$c\,\partial u/\partial t$, and the two forms agree to the last bit; there is no cost to paying
for the general case. Where $c$ does vary they differ, and it is the conservative form that a
