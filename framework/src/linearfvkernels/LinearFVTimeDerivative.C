...
LinearFVTimeDerivative::validParams()
{
  ...
  params.addParam<bool>(
      "conservative_form",
      true,
      "Whether to assemble the transient of the product, d(cu)/dt, rather than the multiplier "
      "times the transient of the variable, c du/dt. The conservative form is the default "
      "because it is the more general of the two: where the multiplier does not vary in time "
      "every state of it is the same value, the product form collapses to c du/dt exactly, and "
      "the two agree to the last bit. Where it does vary they differ, and it is the conservative "
      "form that a conservation law asks for, the non-conservative one being short of u dc/dt. "
      "Set this to false to recover the multiplier held outside the derivative.");
  return params;
}
