...
########################################################################################################
# The compressing counterpart of conservative-transient-interfacial-area.i: the dispersed phase
# density falls to a third of its initial value rather than tripling, so that the expansion term of
# the interfacial area equation changes sign.
#
# THE TERM UNDER TEST. The source kernel carries -(1/3) d(rho_d)/dt on the diagonal. Under
# expansion that is positive and strengthens it; under compression it is negative and would weaken
# it, so the kernel takes the strengthening part implicitly and lags the rest. The conserved
# integral is invariant either way, which is what this case asserts, and the two treatments agree
# at convergence by construction.
#
...
rho_d = 500.0
rho_d_final_factor = 0.3333333333333333
