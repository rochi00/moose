...
# z/D = 6.00, 30.3 and 53.5. The domain begins at the first of them rather than at the physical
# pipe inlet, so that the concentration imposed there is imposed at the station whose measured value
# it is. Starting at z/D = 0 develops the field for six diameters before the first station and so
# counts the entry length twice, once in the experiment and once here, which inflates the computed
# concentration at that station by between 0.8 and 1.6 per cent and every station downstream with
# it. With 108 axial cells over the remaining 54 diameters the cell width is exactly D/2,
...
# interpolation discards. A sideset at 54.0 is kept as well, half a diameter past the probe, so that
# the bias of reporting the outlet there instead can be measured rather than estimated.
#
...
[Mesh]
  ...
  [gen]
    ...
    ny = 16
  []
  # Cross-sections on which the area averages are taken. The first measured station, z/D = 6.00, is
  # the inlet plane itself now, so 'left' is that station and no sideset has to be cut for it.
  # zD53p5 is the last measured station; zD29p5, zD30 and zD30p5 bracket the measured station at
  # 30.3; zD54 is the legacy outlet station, retained only to quantify the offset it introduced.
  [station_30]
