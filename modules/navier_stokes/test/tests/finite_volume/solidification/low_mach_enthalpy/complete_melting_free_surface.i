...
[FunctorMaterials]
  ...
  [uniform_pcm_heating]
    type = ParsedFunctorMaterial
    # Uniform material-specific heating drives the initially solid PCM completely through the
    # mushy interval while leaving the gas unforced.
    property_name = uniform_pcm_heating
    functor_names = alpha
    expression = '1160 * alpha'
  []
  [pcm_mass_density]
    type = ParsedFunctorMaterial
    # The expected invariant is integral(alpha * rho_pcm) = 0.5 * rho_solid = 4.
    property_name = pcm_mass_density
    functor_names = 'alpha low_mach_liquid_fraction'
    expression = 'alpha * (8 + (6 - 8) * low_mach_liquid_fraction)'
  []
  [pcm_phase_density]
    type = ParsedFunctorMaterial
    property_name = pcm_phase_density
    functor_names = low_mach_liquid_fraction
    expression = '8 + (6 - 8) * low_mach_liquid_fraction'
  []
  [pcm_liquid_volume_density]
    type = ParsedFunctorMaterial
    property_name = pcm_liquid_volume_density
    functor_names = 'alpha low_mach_liquid_fraction'
    expression = 'alpha * low_mach_liquid_fraction'
  []
[]
...
[AuxKernels]
  ...
  [pcm_mass_aux]
    type = FunctorAux
    variable = pcm_mass_aux
    functor = pcm_mass_density
    execute_on = 'TIMESTEP_END FINAL'
  []
  [pcm_liquid_volume_aux]
    type = FunctorAux
    variable = pcm_liquid_volume_aux
    functor = pcm_liquid_volume_density
    execute_on = 'TIMESTEP_END FINAL'
  []
[]
...
[LinearFVKernels]
  ...
  [heat_source]
    type = LinearFVSource
    variable = T
    source_density = uniform_pcm_heating
  []
  [low_mach_divergence]
