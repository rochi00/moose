...
T_cold = 0.97
T_liquid = 1.10
initial_liquid_height = 0.70
liquid_column_width = 0.80
domain_width = 6
mu_gas = 1
mu = 1
...
[Functions]
  [initial_alpha]
    type = ParsedFunction
    expression = 'if(x < liquid_column_width & y < initial_liquid_height, 1, 0)'
    symbol_names = 'liquid_column_width initial_liquid_height'
    symbol_values = '${liquid_column_width} ${initial_liquid_height}'
  []
  [initial_fl]
    type = ParsedFunction
    expression = 'if(x < liquid_column_width & y < initial_liquid_height, 1, 0)'
  ...
  [initial_T]
    type = ParsedFunction
    expression = 'if(x < liquid_column_width & y < initial_liquid_height, T_liquid, T_gas)'
    symbol_names = 'liquid_column_width initial_liquid_height T_liquid T_gas'
    symbol_values = '${liquid_column_width} ${initial_liquid_height} ${T_liquid} ${T_gas}'
  []
  [initial_rho_h]
    type = ParsedFunction
    expression = 'if(x < liquid_column_width & y < initial_liquid_height, rho_liquid * cp_liquid * T_liquid, rho_gas * cp_gas * T_gas)'
    symbol_names = 'liquid_column_width initial_liquid_height rho_liquid rho_gas cp_liquid cp_gas T_liquid T_gas'
    symbol_values = '${liquid_column_width} ${initial_liquid_height} ${rho_liquid} ${rho_gas} ${cp_liquid} ${cp_gas} ${T_liquid} ${T_gas}'
  []
  [bottom_rho_h]
    type = ParsedFunction
...
[Executioner]
  ...
  volume_fraction_max_courant = 0.1
