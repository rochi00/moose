...
LinearWCNSFV2PInterfaceAreaSourceSink::computeCoefficients()
{
  ...
  //
  // The size also has to be one the closures can be evaluated at. The coalescence prefactor
  // divides by db^(11/3), which underflows to zero below db of about 8e-89 and leaves the
  // prefactor infinite. The breakage term then multiplies that infinity by an exponential which
  // has itself flushed to zero, and the source comes out as a NaN. Both sources tend to zero in
  // this limit in any case, s_rc as f_d^(1/3) and s_ti because the exponential beats its power
  // law, so declining to evaluate them here returns the value they approach rather than
  // approximating it. An implicit upwind solve leaves a cell in that range wherever the phase
  // front has yet to reach it, the solution there decaying geometrically away from the front, so
  // this is the ordinary state of the domain ahead of the front rather than an unusual one.
