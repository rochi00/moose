...
#include "LinearWCNSFVMultiPhaseMomentumFlux.h"
...
registerMooseObject("NavierStokesApp", LinearWCNSFVMultiPhaseMomentumFlux);

InputParameters
LinearWCNSFVMultiPhaseMomentumFlux::validParams()
{
  ...
  params.addClassDescription("Represents the matrix and right hand side contributions of the "
                             "stress and advection terms of the two-phase momentum equation.");
...
LinearWCNSFVMultiPhaseMomentumFlux::LinearWCNSFVMultiPhaseMomentumFlux(const InputParameters & params)
...
Real
LinearWCNSFVMultiPhaseMomentumFlux::computeElemMatrixContribution()
{
...
Real
LinearWCNSFVMultiPhaseMomentumFlux::computeNeighborMatrixContribution()
{
...
Real
LinearWCNSFVMultiPhaseMomentumFlux::computeElemRightHandSideContribution()
{
  return computeInternalStressRHSContribution() * _current_face_area;
}

Real
LinearWCNSFVMultiPhaseMomentumFlux::computeNeighborRightHandSideContribution()
{
  return -computeInternalStressRHSContribution() * _current_face_area;
}

Real
LinearWCNSFVMultiPhaseMomentumFlux::computeBoundaryMatrixContribution(const LinearFVBoundaryCondition & bc)
{
...
Real
LinearWCNSFVMultiPhaseMomentumFlux::computeBoundaryRHSContribution(const LinearFVBoundaryCondition & bc)
{
  const auto * const adv_diff_bc = static_cast<const LinearFVAdvectionDiffusionBC *>(&bc);
  mooseAssert(adv_diff_bc, "This should be a valid BC!");
...
Real
LinearWCNSFVMultiPhaseMomentumFlux::computeInternalAdvectionElemMatrixContribution()
{
  return _advected_interp_coeffs.first * _face_mass_flux;
}

Real
LinearWCNSFVMultiPhaseMomentumFlux::computeInternalAdvectionNeighborMatrixContribution()
{
  return _advected_interp_coeffs.second * _face_mass_flux;
}

Real
LinearWCNSFVMultiPhaseMomentumFlux::computeInternalStressMatrixContribution()
{
  ...
  if (!_cached_matrix_contribution)
  {
    ...
    _stress_matrix_contribution = _mu(face_arg, determineState()) * _alpha(face_arg, determineState()) / d;
...
Real
LinearWCNSFVMultiPhaseMomentumFlux::computeInternalStressRHSContribution()
{
  ...
  if (!_cached_rhs_contribution)
  {
    ...
    if (_dim > 1 && _use_nonorthogonal_correction)
    {
      ...
      _stress_rhs_contribution +=
          _mu(face_arg, state_arg) * _alpha(face_arg, state_arg) *
    ...
    else if (_use_deviatoric_terms)
    {
      ...
      _stress_rhs_contribution += _mu(face_arg, state_arg) * _alpha(face_arg, state_arg) *
...
Real
LinearWCNSFVMultiPhaseMomentumFlux::computeStressBoundaryMatrixContribution(
    const LinearFVAdvectionDiffusionBC * bc)
{
  ...
  if (!bc->includesMaterialPropertyMultiplier())
  {
    ...
    grad_contrib *= _mu(face_arg, determineState()) * _alpha(face_arg, determineState());
  }
...
Real
LinearWCNSFVMultiPhaseMomentumFlux::computeStressBoundaryRHSContribution(
    const LinearFVAdvectionDiffusionBC * bc)
{
  const auto face_arg = singleSidedFaceArg(_current_face_info);
  ...
  if (_use_nonorthogonal_correction)
  {
    ...
    grad_contrib += _mu(face_arg, determineState())
                    * _alpha(face_arg, determineState())
...
Real
LinearWCNSFVMultiPhaseMomentumFlux::computeAdvectionBoundaryMatrixContribution(
    const LinearFVAdvectionDiffusionBC * bc)
{
...
Real
LinearWCNSFVMultiPhaseMomentumFlux::computeAdvectionBoundaryRHSContribution(
    const LinearFVAdvectionDiffusionBC * bc)
{
  // We support internal boundaries too so we have to make sure the normal points always outward
  const auto factor = (_current_face_type == FaceInfo::VarFaceNeighbors::ELEM ? 1.0 : -1.0);
...
void
LinearWCNSFVMultiPhaseMomentumFlux::setupFaceData(const FaceInfo * face_info)
{
