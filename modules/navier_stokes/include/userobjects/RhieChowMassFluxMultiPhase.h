 ...
 */
class RhieChowMassFluxMultiPhase : public RhieChowFaceFluxProvider, public NonADFunctorInterface
{
public:
  static InputParameters validParams();
  RhieChowMassFluxMultiPhase(const InputParameters & params);

  /// Get the face velocity times density (used in advection terms)
  Real getMassFlux(const FaceInfo & fi) const;

  /// Get the face velocity times density not weighted with the phase fraction (used in advection terms)
  Real getUnweightedMassFlux(const FaceInfo & fi) const;
...
protected:
  ...
  /// A suffix to add to the RC objects for multiple phases
  const std::string _prop_suffix;
...
template <typename VarType>
void
RhieChowMassFluxMultiPhase::checkBlocks(const VarType & var) const
{
